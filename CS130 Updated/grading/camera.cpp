...
vec3 Camera::World_Position(const ivec2& pixel_index)
{
    ...
    //Starting World Position of input pixel with film position

    //Calculating horizontal and vertical offsets of the pixel in the film
    vec2 temp = Cell_Center(pixel_index);
    
    result += horizontal_vector*temp[0] + vertical_vector*temp[1] + film_position;
    //result += temp[0]*horizontal_vector + temp[1]*vertical_vector + film_position;
